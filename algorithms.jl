...
function stochAlgorithm2Bregman(A::Array{Float64,2}, proj::Function,
    z0::Array{Float64,1}, w0::Array{Float64,1}, γ::Float64, η::Float64, p::Float64, b::Int64, K::Int64,
    ...
    for s in 1:S
        for k in 1:K
            update_w = update_wArray[k]
            i, j = arrayI[:, k], arrayJ[:, k]
            x, y, z, Z, X_old, Y_old, wx, wy, wx_old, wy_old, w_next, W_next, energy, epoch, epoch_count =
            stochAlgorithm2Bregman_update(A, k, proj, x, y, z, W_, Z, X_old, Y_old, wx, wy, wx_old, wy_old, w_next, W_next,
                                            energy, epoch, epoch_count, γ, η, b, i, j, cheap_update,
                                            update_w, rows_weights, columns_weights)
        end
        W_ = W_next
        wx, wy = w_next[1:n], w_next[(n + 1):end]
        gap = maximum(A * w_next[1:n]) - minimum(A' * w_next[(n + 1):end])
        #print("$s, $gap \n")
        append!(energy, gap)
...
function stochAlgorithm2Bregman_update(A, k, proj, x, y, z, W_, Z, X_old, Y_old, wx, wy, wx_old, wy_old, w_next, W_next, energy, epoch,
        epoch_count, γ, η, b, i, j, cheap_update, update_w, rows_weights, columns_weights)
    
    X, Y = Z[1:n], Z[(n+1):end]
    X_, Y_ = zeros(n), zeros(m)
    setindex!(X_, (Y - wy + Y - Y_old)[j], j) 
    setindex!(Y_, (X - wx + X - X_old)[i], i)    
    delta_x = 1 / b .* (A' * (Y_ ./ rows_weights)) .+ A' * wy 
    delta_y = 1 / b .* (-A * (X_ ./ columns_weights)) .- A * wx
    
    X_old, Y_old = copy(X), copy(Y)
    
    X = X .+ γ .* (W_[1:n] .- X) .- η .* delta_x 
    Y = Y .+ γ .* (W_[(n+1):end] .- Y) .- η .* delta_y 

    epoch_count += cheap_update * b
    
    Z = vcat(X, Y)
    z = [softmax(X); softmax(Y)]
    
    w_next = (z .+ (k-1) .* w_next) ./ k

    W_next = (Z .+ (k-1) .* W_next) ./ k
    #print(norm(X - X_old), " ", norm(Y - Y_old), "\n")
    return x, y, z, Z, X_old, Y_old, wx, wy, wx_old, wy_old, w_next, W_next, energy, epoch, epoch_count
end